...
class FletEasyX:
    ...
    def __init__(
        ...
        self.__data.view = (
            self.__view_data_config() if self.__view_data is not None else None
        )
        ...
        if callable(pagesy.view) and not isinstance(pagesy.view, type):
            view = (
                self.__page.run_task(
                    pagesy.view, self.__data, **self.__data.url_params
                ).result()
    ...
    def _verify_url(
        ...
        if custom_types and type_ not in custom_types:
            raise ValueError(
                f"A custom data type is not being used: {custom_types.keys()}"
            )
        ...
        result = {
            name: parser(match.group(i + 1))
            for i, (name, parser) in enumerate(segments)
        }
