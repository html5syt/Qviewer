...
def _handle_decode_errors(
    jwt: str, data: Datasy, key_login: str
...
def decode(key_login: str, data: Datasy) -> Dict[str, Any] | bool:
    ...
    * `data` : Instance object of the `Datasy` class.
    """
    assert (
        data.secret_key is not None
    ), "set the 'secret_key' in the class parameter (FletEasy)."
    try:
        ...
        raise LoginError(
            "Use the 'decode_async' method instead of 'decode'. | More details:", e
        )
...
async def decode_async(key_login: str, data: Datasy) -> Dict[str, Any] | bool:
    ...
    * `data` : Instance object of the `Datasy` class.
    ...
    try:
        return _handle_decode_errors(
            jwt=await data.page.client_storage.get_async(key_login),
            data=data,
            key_login=key_login,
        )
    except TimeoutError as e:
        raise LoginError(
            "Use the 'decode' method instead of 'decode_async'. | More details:", e
        )
